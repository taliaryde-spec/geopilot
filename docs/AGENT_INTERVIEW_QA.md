...
Agent 和 GIS 执行仍是同步 HTTP，因此页面只能等待最终响应，无法实时显示步骤、取消任务或断线恢复；没有上传安全、多个图层控制、大 GeoJSON 简化/瓦片、浏览器 E2E、认证和无障碍审计。Job + SSE 能解决长任务和实时状态，但它属于产品工程支线。GeoPilot 当前首先服务于 Agent 组件学习和求职准备，所以下一主线改为 Prompt/结构化输出实验，核心组件补齐后再回来解决这些产品缺口。

### 78. Job 和 SSE 是什么，为什么现在不把它们作为下一阶段？

Job 是把耗时的 Agent/GIS 操作放到后台任务中，让 HTTP 立即返回任务 ID；SSE 是服务端通过一个持续连接向浏览器单向推送进度。它们能改善页面等待、取消和断线体验，但不改变模型如何理解指令、调用工具、检索知识或使用记忆。此前把它们列为下一阶段是优先级偏移；对本项目的学习目标，Prompt、Function Calling、Agent 模式、RAG/Context 和 MCP 更应该先系统实践。

### 79. 项目已经有 Prompt 和 RAG，为什么还要回头做组件实验？

“代码存在”不等于“真正掌握”。当前 Prompt 0.8.0 和 RAG 链路能运行，但 Prompt 还缺基础版/结构化版/Few-shot 的同题对照，RAG 还缺生成 Faithfulness、引用正确率和无答案拒答。回头补的是可复现实验、失败分析和选型依据，不是重复抄代码。面试官真正会追问为什么这样设计、哪个指标改善、什么情况下换方案，只有实验闭环才能回答。

### 80. 纠偏后每个 Agent 组件怎样学才算完成？
...
- 新增 Job/SSE 定义与优先级、为什么已有组件仍需补实验、组件学习八件套共 3 道项目化问答。
